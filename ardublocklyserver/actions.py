...
def load_arduino_cli(sketch_path):
    ...
    if success:
        ...
        ide_directory = os.path.dirname(settings.compiler_dir)
        arduino_cli = os.path.join(ide_directory, "resources", "app", "lib", "backend", "resources", "arduino-cli.exe")
        cli_command = [arduino_cli]
        global cli_commands_setup 

        if cli_commands_setup:
            cli_command.extend(['core', 'update-index', '&&'])
            cli_command.append(arduino_cli)
            cli_command.extend(['core', 'install', 'arduino:avr', '&&'])
            cli_command.append(arduino_cli)
            cli_command.extend(['config', 'init', '--overwrite', '&&'])
            cli_command.append(arduino_cli)
            cli_command.extend(['config', 'set', 'library.enable_unsafe_install', 'true', '&&'])
            cli_command.append(arduino_cli)
            cli_command.extend(['lib', 'install', '--git-url', 'https://github.com/SolisTecnologia/MiniSobot-ArduinoLibrary.git', '&&'])
            cli_command.append(arduino_cli)
            cli_commands_setup = False
        ...
        if settings.load_ide_option == 'open':
            ...
            for line in lines:
                    ...
                    elif 'Global variables use' in line:
                        std_out += line

            err_out = err_out.decode('utf-8')
            # std_out = six.u(bytes(std_out))
